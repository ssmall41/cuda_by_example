...
// The module here does NOT use constant memory.
...
__global__ void compute_dot_prods(float* vectors_data, float* dev_core_vector, float* dots, int num_vectors)
{
...
int main(int argc, char* argv[])
{
	...
	float* dev_core_vector;
	cudaMalloc(&dev_core_vector, SIZE*sizeof(float));
	for(i=0;i<SIZE;i++)
		temp_core[i] = i - SIZE / 2;
	cudaMemcpy(dev_core_vector, temp_core, sizeof(float)*SIZE, cudaMemcpyHostToDevice);
	free(temp_core);
	...
	compute_dot_prods<<<num_blocks, num_threads>>>(dev_vectors_data, dev_core_vector, dev_dots, num_vectors);
